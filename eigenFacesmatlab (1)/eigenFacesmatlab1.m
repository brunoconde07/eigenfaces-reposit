...
%
    ...
    subplot(2,3,1),imshow(mic1,'Initialmagnification','fit');;title('mic1')


% shelly1
shelly1 = imread('shelly\shelly1.JPG','jpg');
shelly1=rgb2gray(shelly1);
shelly1=imresize(shelly1,[N N] );
subplot(2,3,2),imshow(shelly1,'Initialmagnification','fit');;title('shelly1')


% linoy1
linoy1 = imread('linoy\linoy1.JPG','jpg');
linoy1=rgb2gray(linoy1);
linoy1=imresize(linoy1,[N N] );
subplot(2,3,3),imshow(linoy1,'Initialmagnification','fit');;title('linoy1')


% libi1
libi1 = imread('libi\libi1.JPG','jpg');
libi1=rgb2gray(libi1);
libi1=imresize(libi1,[N N] );
subplot(2,3,4),imshow(libi1,'Initialmagnification','fit');;title('libi1')

% extra face
extraface1 = imread('extra\extraface1.JPG','jpg');
extraface1=rgb2gray(extraface1);
extraface1=imresize(extraface1,[N N] );
subplot(2,3,5),imshow(extraface1,'Initialmagnification','fit');;title('extraface1')

% m_gr.jpg
extraface2 = imread('gr\gr1.JPG','jpg');
extraface2=rgb2gray(extraface2);
extraface2=imresize(extraface2,[N N] );
subplot(2,3,6),imshow(extraface2,'Initialmagnification','fit');;title('extraface2')
...
for k=1:M
    ...
    subplot(2,3,k),imshow(avImg,'Initialmagnification','fit');title('average')
    pause(1)
end
pause(2)
